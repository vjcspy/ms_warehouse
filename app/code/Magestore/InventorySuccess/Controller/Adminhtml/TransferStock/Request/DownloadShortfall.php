...
class DownloadShortfall extends \Magestore\InventorySuccess\Controller\Adminhtml\TransferStock\AbstractRequest
{
    ...
    public function execute()
    {
        ...
        $data = array(
            array('Name','SKU','REQUESTED QTY', 'QTY DELIVERED', 'QTY RECEIVED')
        );
    ...
    public function generateSampleData() {
        ...
        foreach ($productCollection as $product) {
            if($product->getData('qty_delivered') != $product->getData('qty_received')){
                $data[]= array($product->getData('product_name'), $product->getData('product_sku'), $product->getData('qty'), $product->getData('qty_delivered'), $product->getData('qty_received'));
            }
        }
        return $data;
    }
