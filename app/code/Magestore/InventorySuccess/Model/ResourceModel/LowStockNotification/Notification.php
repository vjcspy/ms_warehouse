...
class Notification extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    public function getProductNotificationByWarehouse($rule, $productIds, $warehouseIds)
    {
        ...
        foreach ($warehouseIds as $warehouseId) {
            ...
            if ($lowstockThresholdType == \Magestore\InventorySuccess\Model\LowStockNotification\Rule::TYPE_LOWSTOCK_THRESHOLD_SALE_DAY) {
                $collection->getSelect()->join(
                    ['warehouse_shipment_item' => $this->_coreResource->getTableName('os_warehouse_shipment_item')],
                    ...
                    [
                        ...
                        'availability_date' => new \Zend_Db_Expr("DATE_ADD(CURDATE(),INTERVAL(FLOOR(warehouse_product.total_qty / (SUM(warehouse_shipment_item.qty_shipped) / {$salesPeriod}))) DAY)"),
                        'availability_days' => new \Zend_Db_Expr("FLOOR(GREATEST(warehouse_product.total_qty / (SUM(warehouse_shipment_item.qty_shipped) / {$salesPeriod}),0))")
                    ]
                )
                    ...
                    ->having("GREATEST((SUM(warehouse_shipment_item.qty_shipped) / {$salesPeriod} * {$lowstockThreshold} - warehouse_product.total_qty),0) > ?", 0);
            }
            if ($collection->count() > 0) {
