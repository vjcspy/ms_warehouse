    ...
    implements WarehouseInterface, OptionSourceInterface, StockActivityInterface, PermissionTypeInterface
{
    ...
    public function addProductsInlineToCatalog($postData = [])
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $productCollection = $objectManager->create('Magento\CatalogInventory\Model\Spi\StockRegistryProviderInterface');
        foreach ($postData as $warehouseId => $productIds) {
            foreach($productIds as $productId){
                $stockItem = $productCollection->getStockItem($productId,0);
